...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_metode)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO alternatif (nama_metode, deskripsi) VALUES (?, ?)");
            $stmt->execute([$nama_metode, $deskripsi]);
            
            header("Location: index.php?success=1");
            exit();
        } catch (PDOException $e) {
            $error = 'Gagal menambahkan metode: ' . $e->getMessage();
        }
    }
}
...
<div class="bg-white shadow rounded-lg p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Metode Pengajaran</h1>
    ...
    <form action="" method="POST" class="space-y-6">
        <div>
            ...
            <input type="text" name="nama_metode" id="nama_metode" required
                ...
                value="<?php echo htmlspecialchars($_POST['nama_metode'] ?? ''); ?>">
        </div>
